...
# #----------------------------------------------------------------------------------------------------------
	if each_run == True: 
		...
		# RidgeCV to get betas
			...
			#shell()
			ridge_fit = RidgeCV(alphas = np.linspace(1,2500,2500) , fit_intercept = False, normalize = True )
			#ridge_fit = RidgeCV(alphas = np.linspace(1,800,800) , fit_intercept = False, normalize = True )
			#ridge_fit = RidgeCV(alphas = np.linspace(1,610, 610) , fit_intercept = False, normalize = True )
			#ridge_fit = RidgeCV(alphas = np.linspace(1,607, 607) , fit_intercept = False, normalize = True )
...
# #----------------------------------------------------------------------------------------------------------
	...
	#----------------------------------------------------------------------------------------------------------
		...
		# when concatenating all runs (4 runs), if the range is (1,1000,1000), the alpha value is 70. (this takes 20 mins to run)
		# w
		...
		for voxelii, voxel in enumerate(voxels):
			...
			f.savefig( '%s_%s_best%s_#%s_r2_%s_RidgeCV_moco.png'%(subname, data_type, str(20-voxelii), str(voxel[0]), str(voxel[1])))
