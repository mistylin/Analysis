...
#----------------------------------------------------------------------------------------------------------
	...
	design_matrix_col = np.hstack([  model_BOLD_timecourse_col_ab ])# np.ones((fmri_data.shape[1],1)), model_BOLD_timecourse_col_ab ])#, moco_params])
	...
	plt.hist(r_squared_col[ ~np.isnan(r_squared_col) ] )
	...
	for voxelii, voxel in enumerate(voxels):
		...
		f2.savefig( '%s_%s_2colDimen_best%s_#%s_r2_%s_RidgeCV_moco.png'%(subname, data_type, str(20-voxelii), str(voxel[0]), str(voxel[1])))
