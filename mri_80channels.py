...
# #----------------------------------------------------------------------------------------------------------
	...
	for fileii, filename_beh in enumerate(target_files_beh):
		...
		empty_end = 15
		# number_of_stimuli = 8?????
		number_of_elements = 40# (5*8)  instead of 48 (6*8)
		...
		color_a_values = [75, 53, 0, -53, -75 ]
		color_b_values = [75, 53, 0, -53, -75 ]

# 8 --> 6, therefore 2 repititions 53, -53
		color1 = ( 75, 0) # although it's 0 when printing out the value, code it as -0.001, to make it different from color 5's b value
		color2 = ( 53, 53)
		color3 = ( 0, 75)
		color4 = ( -53, 53)
		color5 = ( -75, 0)
...
# take out the intercept for both
	...
	plt.savefig( '%s_80ch_%s_%s_r_suqareds_RidgeCV.jpg'%(subname, data_type, ROI))
	...
	for voxelii, voxel in enumerate(voxels):
		...
		beta_matrix_a = betas_z[0:number_of_elements, voxel[0]].reshape(len(orientations),len(color_a_values))
...
# another dimension
		...
		beta_matrix_b = betas_z[number_of_elements:number_of_elements*2, voxel[0]].reshape(len(orientations),len(color_a_values))  # 8 rows-oientation, 6 columns.
		...
		# shell()
		f.savefig( '%s_80ch_%s_%s_best%s_#%s_r2_%s_RidgeCV.png'%(subname, data_type, ROI, str(20-voxelii), str(voxel[0]), str(voxel[1])))
...
f = plt.figure(figsize = (12,12))
gs=GridSpec(len(orientation), len(color_a_values)) # 8,5
s1 = f.add_subplot(len(orientation), len(color_a_values), 1)

for oriii in len(orientation):
	for colii in len(color_a_values):
		s1=f.add_subplot(gs[ oriii , colii])
		plt.plot (lines)
...
def prepare_lines (point, angle, length):
