...
sublist = [('sub-004', True, False)]#[('sub-001', False, True) ]# , , [('sub-002', True, False)], [('sub-004', True, False)]
...
# #----------------------------------------------------------------------------------------------------------
	...
	# GLM to get betas
		...
		if type_contrasts == 'full':
			...
			for i in range(n_contrasts):
				...
				# orientation
				c = np.r_[0,np.repeat(a, 8, axis = 0).T, c_moco, c_key_press] 
		...
		elif type_contrasts == 'color':

			n_contrasts = 8
			t = np.zeros((n_voxels, n_contrasts))
			p = np.zeros((n_voxels, n_contrasts))

			for i in np.arange(0, 64, 8): # 0,8,16,24,32,40,48,56
				...
				# orientation
				c = np.r_[0, a, c_moco, c_key_press] 

				design_var = c.dot(np.linalg.pinv(design_matrix.T.dot(design_matrix))).dot(c.T)
				SE_c = np.sqrt(_sse * design_var)
				
				column_index = 0
				t[:, column_index] = betas.dot(c) / SE_c  # SE_c (10508,)
				p[:, column_index] = scipy.stats.t.sf(np.abs(t[:,column_index]), df)*2
	...
	t_mean = np.mean(np.array(t_mean), axis = 0) # average across runs
	# t_mean shape: (5638,64)
	
	# t_ori = np.zeros((n_voxels, 9))
	# t_col = np.zeros((n_voxels, 9))

	# position_cen = 2

	# for vi in range(n_voxels):
	# 	t_matrix = t_mean [vi,:].reshape(8,8)
	...
	# 	# center
	# 	t_matrix_cenRow = np.roll(t_matrix, (position_cen-t_pre_index[0]), axis = 0)
	# 	t_matrix_cen= np.roll(t_matrix_cenRow, (position_cen-t_pre_index[1]), axis = 1)
	...
	# 	t_ori[vi, :] = t_matrix_cir[position_cen,:]
	# 	t_col[vi, :] = t_matrix_cir[:,position_cen]

	# t_ori_mean = np.mean(t_ori, axis = 0)
	# t_col_mean = np.mean(t_col, axis = 0)
	# # shell()

	# sd = np.array([np.std(t_ori, axis = 0), np.std(t_col, axis = 0)])
	# n = n_voxels
	...
	# plt.close()
	# f1.savefig( '%s_%s_%s_%s_tValues_overVoxels.png'%(subname, ROI, data_type, regression))
	# print 'plotting t_values over all voxels'
	...
	if type_contrasts == 'ori' or type_contrasts == 'color':
		...
		for voxelii, voxel in enumerate(voxels):
			...
			s = f.add_subplot(111)
			plt.plot( t_mean [vi] )
			s.set_title(type_contrasts, fontsize = 10)
			plt.close()

			f.savefig( '%s_%s_%s_%s_%s_best%s_#%s_tValues_%s.png'%(subname, ROI, type_contrasts, data_type, regression, str(20-voxelii), str(voxel[0]), str(voxel[1])))

			print "plotting_best%s"%(str(n_best-voxelii))
		...
		if position_cen == 2: 
			...
			for voxelii, voxel in enumerate(voxels):
				...
				t_matrix = t_mean [voxel[0]].reshape(8,8)
				...
				t_ori = t_matrix_cir[position_cen,:]
				t_col = t_matrix_cir[:,position_cen]

				t_oriBestVox[voxelii,:] = t_ori
				t_colBestVox[voxelii,:] = t_col
				...
				# plt.plot(t_ori ) # ---orientation
				# s3.set_title('orientation', fontsize = 10)


				# s4 =f.add_subplot(gs[3:,-2]) # Second row, span all columns
				# roate_90_clockwise( t_col  )
				# s4.set_title('color', fontsize = 10)


				# plt.close()
				# f.savefig( '%s_%s_%s_%s_%s_best%s_#%s_tValues_%s.png'%(subname, ROI, type_contrasts, data_type, regression, str(n_best-voxelii), str(voxel[0]), str(voxel[1])))
			...
			sd = np.array([np.std(t_oriBestVox, axis = 0), np.std(t_colBestVox, axis = 0)])
			n = n_best
			yerr = (sd/np.sqrt(n))*1.96
			...
			plt.plot(t_oriBestVox_mean)
			plt.errorbar(range(0,9), t_oriBestVox_mean, yerr= yerr[0])
			s1.set_title('orientation', fontsize = 10)

			s2 = f2.add_subplot(212)
			plt.plot(t_colBestVox_mean)
			plt.errorbar(range(0,9), t_colBestVox_mean, yerr= yerr[1])
			s2.set_title('color', fontsize = 10)

			f2.savefig( '%s_%s_%s_%s_tValues_%sVoxels.png'%(subname, ROI, data_type, regression, n_best))
			print 'plotting t_values over 100 voxels'
			...
			for voxelii, voxel in enumerate(voxels):
				...
				if t_pre_index.size == 2:
					...
					t_pre_index = np.array([t_pre_index[0][0], t_pre_index[1][0]])
					print 'more than one preferred stimulus', voxel[0], t_pre_index[0].shape
				...
				t_ori = t_matrix_cir[t_pre_index[0],:]
				t_col = t_matrix_cir[:,t_pre_index[1]]
