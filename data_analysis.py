...
def plot_staircase(csv_files, subname):
	...
	color_staircase = np.abs(trial_color)
	ori_staircase = np.abs(trial_ori)
	...
	pl.plot(color_staircase)
	#pl.ylim([0, 50])
	sn.despine(offset=10)
	s4.set_title('staircase color', fontsize = 20)

	s5 = f.add_subplot(235)
	pl.plot(ori_staircase)
	#pl.ylim([0, 5])
	sn.despine(offset=10)
	s5.set_title('staircase orientation', fontsize = 20)

	#pl.show()
	pl.savefig('/Users/xiaomeng/disks/Aeneas_Home/Analysis/pdfs/%s_color_ori_staircase_plot.pdf'%(subname))
